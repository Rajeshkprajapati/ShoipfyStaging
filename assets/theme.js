...
Shopify.getCart = function(callback) {
  jQuery.getJSON('/cart.js', function (cart, textStatus) {
    if ((typeof callback) === 'function') {
      //alert('Test alert on getcart');
      //console.log(cart);
      //if(cart.item_count<2){
      	callback(cart); 
//       }else{      
//         alert('Not allowed to add more than one items'); 
//       }
    }
    else {
      Shopify.onCartUpdate(cart);
...
Shopify.getProduct = function(handle, callback) {
  jQuery.getJSON('/products/' + handle + '.js', function (product, textStatus) {
    if ((typeof callback) === 'function') {
      alert('Test alert on get Product');
      callback(product);
    }
    else {
...
Shopify.changeItem = function(line, quantity, callback) {
  var params = {
    ...
    success: function(cart) {
      if ((typeof callback) === 'function') {
        //alert('Test on change item');
        console.log(line,quantity,cart);
        callback(cart);
      }
      else {
        Shopify.onCartUpdate(cart);
  ...
  if(quantity<2){
    ...
    alert('Not Allowed to add more than one item');
  }
